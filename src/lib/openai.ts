...
export async function getAIResponse(
  ...
  try {
    ...
    if (isFollowUp && documentId) {
      ...
      const rankings = JSON.parse(completion.choices[0]?.message?.content || '{}').rankings;
...
${isFollowUp ? 'This is a follow-up question. Consider previous context.' : 'This is a new question.'}`
        },
        {
          role: 'user',
    ...
    if (response.includes('not available in the current document')) {
      return isFollowUp
        ? {
            bestAnswer: 'The answer is not available in the current document. If you want to search across all documents, please start a new chat.',
    ...
    const sections = {
      ...
      reference: (response.match(/\[Reference\](.*?)(?=\[|$)/s) || [])[1]?.trim(),
    ...
    throw error instanceof Error ? error : new Error('Error processing your request. Please try again.');
  }
}
  ...
  try {
    ...
    const completion = await openaiClient.chat.completions.create({
      messages: [
        { 
          role: 'system', 
          content: 'Provide a brief, focused summary of the main points in 2-3 sentences.' 
        },
        { role: 'user', content }
      ],
      model: 'gpt-3.5-turbo',
    ...
    console.error('OpenAI API error:', error);
    throw error instanceof Error ? error : new Error('Error analyzing the document. Please try again.');
  }
}
